...
/**
 * Created by Mayur on 09-05-2017.
 */
...
public class CustomRedioButton extends RadioButton
{
    private Typeface tf;
    public  final String DEFAULT_FONT = "fonts/NunitoRegular.ttf";
    public CustomRedioButton(Context context)
    {
        super(context);
        if (!isInEditMode())
            init(null);
    }
    public CustomRedioButton(Context context, AttributeSet attrs)
    {
        super(context, attrs);
        if (!isInEditMode())
            init(attrs);
    ...
    public CustomRedioButton(Context context, AttributeSet attrs, int defStyle)
    {
