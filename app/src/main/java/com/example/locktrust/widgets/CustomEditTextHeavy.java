...
public class CustomEditTextHeavy extends EditText
{
    private Typeface tf;
    public  final String DEFAULT_FONT = "fonts/NunitoBold.ttf";
    public CustomEditTextHeavy(Context context)
    {
        super(context);
        if (!isInEditMode())
            init(null);
    }
    public CustomEditTextHeavy(Context context, AttributeSet attrs)
    {
        super(context, attrs);
        if (!isInEditMode())
            init(attrs);
    ...
    public CustomEditTextHeavy(Context context, AttributeSet attrs, int defStyle)
    {
